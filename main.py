...
def on_created(event):  # Create a function called "on_created" with a parameter called "event" to watch every file created on Windows
    ...
    if file_created[-val:] == suffix:  # if the suffix of the filename correspond to the suffix on variable "suffix" we did the process indented
        file_location = os.getcwd() + event.src_path[1:]  # we get the full path of the file using method getcwd() (https://www.tutorialspoint.com/python/os_getcwd.htm) from library os and let it on variable "file_location"
...
def create_observer(path_directory_to_clean=""):  # Create a function called "create_observer" with parameter path_directory_to_clean which take by default value "" to launch a watcher
    ...
    my_observer.schedule(my_event_handler, path, recursive=go_recursively)  # activate the watcher on directory given on variable "path" using object "my_observer"
...
class App:  # create an object called "App"
    def __init__(self):  # initialise the variables of the object
        ...
        def browsefunc():
            ...
            pathlabel.config(text="Directory to clean " + path_directory)
            #TODO Update observer

        browsebutton = tkinter.Button(self.root, text="Browse directory to clean", command=browsefunc)
        browsebutton.pack()

        pathlabel = tkinter.Label(self.root)
        pathlabel.pack()
